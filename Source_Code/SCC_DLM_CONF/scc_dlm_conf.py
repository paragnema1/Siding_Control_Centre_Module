...
class SccDlmConfRead:
    schema = {
        ...
        "TOTAL_YARD": int,
        "TOTAL_SECTION": int
    } #this is a dictionary describing the schema of scc.config file
    ...
    def read_cfg(self, file_name):
        if path.exists(file_name): #if file exists then it will load in "self.json_data", if not then it will show error
            ...
            sys.exit(1)
        try: #block to check validation of json data (whether json data is maching with schema or not)
            checker = Checker(SccDlmConfRead.schema) #initializing checker object with schema
            result = checker.validate(self.json_data) #validate json data with schema
    ...
    def validate_cfg(self):
        try: #a block to check if any data is not present,empty or whitespace.
...
class DatabaseStruct(NamedTuple): #class to store database from scc.config
    PROVIDER: str
    USER: str
    PASSWORD: str
    HOST: str
